...
import styles from "../../styles/custom-select.module.css";
...
export const CustomSelect = ({
  ...
  return (
    <div className={styles["container-custom-select"]}>
      <label htmlFor={name} id={`${name}-label`}>
        {label} {request && "*"}
      </label>
      <div
        ref={ref}
        className={`${styles["custom-dropdown"]} ${disabled ? `${styles["disabled"]}` : ""} ${
          errorMessage ? `${styles["has-error"]}` : ""
        }`}
        role="combobox"
      ...
      >
        <div
          className={`${styles["dropdown-toggle"]} ${styles["search-mode"]} ${hasFocus ? `${styles["focused"]}` : ""}`}
        ...
        >
          <input
            ...
            placeholder={initValue ? "Seleccione" : "Buscar..."}
            disabled={disabled}
            role="searchbox"
            ...
            aria-activedescendant={
              isOpen && optionsToShow[highlightIndex]
                ? `option-${optionsToShow[highlightIndex].id}`
                : undefined
            }
          />
          <FaChevronDown
        ...
        {isOpen && (
          <ul id={listboxId} className={styles["dropdown-menu"]} role="listbox">
            {optionsToShow.length > 0 ? (
              ...
              <li className={styles["no-results"]}>Sin resultados</li>
            )}
          </ul>
        )}
      </div>
      ...
      {errorMessage && (
        <span className={styles["custom-select-error"]} role="alert">
          {errorMessage}
        </span>
      )}
    </div>
